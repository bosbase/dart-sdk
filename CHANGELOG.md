...
- Added `reuseHTTPClient` `Bosbase` constructor parameter to initialize a single HTTP client and reuse it for all requests, in order to improve slightly the performance by keeping a persistent connection.
  More details you can find in the ["Optional HTTP client reuse" section in the README](https://github.com/bosbase/dart-sdk#optional-http-client-reuse).
...
**⚠️ This release works only with Bosbase v0.23.0+.**

- Added annotation to exclude the deprecated `RecordModel.expand` key from the parent JSON serialization ([bosbase#5946](https://github.com/bosbase/bosbase/issues/5946)).
...
- Added "impersonate" support for superusers to create a non-refreshable auth token for any other auth record:
    ...
    // create a new auth token for the specified user loaded in a new Bosbase client
...
- Added `body`, `query`, `headers` optional arguments to `authWithOAuth2()` ([#62](https://github.com/bosbase/dart-sdk/issues/62)).
...
- ⚠️ Soft-deprecated and aliased `pb.admins` because with Bosbase v0.23+ admins are now stored as regular `_superusers` collection records.
    ```dart
    // before   ->  after
...
- Properly sent json body with `multipart/form-data` requests.
  _This fixes a similar issue described in [js-sdk#274](https://github.com/bosbase/js-sdk/issues/274)._
...
- Fixed `multipart/form-data` body serialization when `null` values are submitted ([#48](https://github.com/bosbase/dart-sdk/issues/48)).


## 0.15.0

- Added `pb.backups.upload(file)` action (_available with Bosbase v0.18.0_).
...
- Added new `AsyncAuthStore` helper class that could be used with any external async persistent layer (shared_preferences, hive, local file, etc.).
  ...
  final pb = Bosbase('http://example.com', authStore: store);
  ```
...
- Modified the original `http.MultipartRequest` to support List field values ([bosbase#2763](https://github.com/bosbase/bosbase/discussions/2763)).


## 0.10.1

- Exposed the internal `Bosbase.httpClientFactory` constructor option to allow users to provide their own `http.Client` implementation as workaround
  for the realtime events on Flutter Web ([#11](https://github.com/bosbase/dart-sdk/issues/11)).


## 0.10.0

- Added `fields` optional parameter to the crud services to limit the returned API fields (_available with Bosbase v0.16.0_).

- Added `pb.backups` service for the new Bosbase backup and restore APIs (_available with Bosbase v0.16.0_).

- Added `filesystem` optional parameter to `pb.settings.testS3()` to allow specifying a filesystem for test - `storage` or `backups` (_available with Bosbase v0.16.0_).
...
- Added `CollectionModel.indexes` field for the new collection indexes support in the upcoming Bosbase v0.14.0.
...
- Added `pb.health.check()` that checks the health status of the Bosbase server (_available in Bosbase v0.10.0_)
...
- Added optional named `expand` argument to all crud functions that returns a `RecordModel` (with v0.8 we now also support [indirect expansion](https://github.com/bosbase/bosbase/issues/312#issuecomment-1242893496)).
...
- Stop sending empty JSON map as body (thanks @rodydavis) [[#7](https://github.com/bosbase/dart-sdk/issues/7)].
