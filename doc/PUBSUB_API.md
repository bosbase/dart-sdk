...
final unsubscribe = await pb.pubsub.subscribe("chat/general", (msg) {
  print("message ${msg.topic} ${msg.data}");
});

// Publish to a topic (resolves when the server stores and accepts it)
final ack = await pb.pubsub.publish("chat/general", {"text": "Hello team!"});
...
- `pb.pubsub.publish(topic, data)` → `Future<PublishAck>`
- `pb.pubsub.subscribe(topic, handler)` → `Future<UnsubscribePubSub>`
- `pb.pubsub.unsubscribe([topic])` → unsubscribe from a topic or all topics.
